<template>
	<view class="send-container">
		...
		<view class="main-content">
			<scroll-view scroll-y class="content-scroll" :scroll-top="scrollTop"
				...
				<view class="content-area">
					...
					<view class="form-card">
						<view class="card-header">
							<view class="title-wrapper">
								...
								<text class="card-title">批量寄件</text>
							</view>
							<view class="add-btn ripple" @click="addNewOrder">
						...
						<view class="form-content">
							<view class="batch-desc">
								<text>您可以添加多个快递订单，统一安排取件和支付</text>
							</view>
						</view>
					</view>

					<!-- 空状态 -->
					<view class="empty-state" v-if="batchOrders.length === 0">
						<image src="/static/img/empty-box.png" mode="aspectFit" class="empty-icon"></image>
						<text class="empty-text">暂无批量寄件订单</text>
						<view class="empty-btn" @tap="addNewOrder">
							<text>添加寄件订单</text>
						</view>
					</view>

					<!-- 批量寄件列表 -->
					<view class="batch-order-list" v-else>
						<view class="batch-order-item" v-for="(order, index) in batchOrders" :key="index"
							@tap="editOrder(index)">
							<!-- 移除了选择框 -->
							<view class="order-content">
								<!-- 添加右上角操作按钮 -->
								<view class="card-actions">
									<view class="action-icon edit" @tap.stop="editOrder(index)">
										<uni-icons type="compose" size="18" color="#3A86FF"></uni-icons>
									</view>
									<view class="action-icon delete" @tap.stop="deleteOrder(index)">
										<uni-icons type="trash" size="18" color="#FF5252"></uni-icons>
									</view>
								</view>

								<!-- 上下布局地址信息 -->
								<view class="order-addresses vertical">
									<!-- 寄件人信息 -->
									<view class="order-address sender-section">
										<text class="address-tag sender">寄</text>
										<view class="address-info">
											<view class="person-info">
												<text class="name">{{ order.senderName }}</text>
												<text class="phone">{{ formatPhone(order.senderMobile) }}</text>
											</view>
											<text
												class="address">{{ order.senderProvince }}{{ order.senderCity }}{{ order.senderDistrict }}{{ order.senderDetailAddress }}</text>
										</view>
									</view>
									...
									<view class="address-divider">
										...
										<view class="divider-arrow">
											<uni-icons type="arrowdown" size="16" color="#CCCCCC"></uni-icons>
										</view>
										<view class="divider-line"></view>
									</view>

									<!-- 收件人信息 -->
									<view class="order-address receiver-section">
										<text class="address-tag receiver">收</text>
										<view class="address-info">
											<view class="person-info">
												<text class="name">{{ order.receiverName }}</text>
												<text class="phone">{{ formatPhone(order.receiverMobile) }}</text>
											</view>
											<text
												class="address">{{ order.receiverProvince }}{{ order.receiverCity }}{{ order.receiverDistrict }}{{ order.receiverDetailAddress }}</text>
										</view>
									</view>
								</view>
								...
								<view class="order-details">
									<view class="order-item-info">
										<text class="item-type">{{ order.itemTypeName || '普通物品' }}</text>
										<text class="item-weight">{{ order.estimatedItemWeight }}kg</text>
									</view>
									<view class="order-price">
										<text>¥{{ order.price.toFixed(2) || 0 }}</text>
									</view>
								</view>
								...
								<view class="insurance-section">
									<view class="insurance-header">
										<view class="insurance-toggle">
											<view class="label-container">
												<uni-icons type="shield" size="16" color="#FF6B01"></uni-icons>
												<text class="label">物品保价</text>
											</view>
											<view v-if="isExpressOrderSupported(index)" class="toggle-switch"
												:class="{ active: order.insured }"
												@click.stop="toggleOrderInsurance(index)">
												<view class="switch-handle"></view>
											</view>
											<view v-else class="not-supported-tip">
												<text>不支持</text>
												<uni-icons type="info" size="14" color="#FFFFFF"
													@click.stop="showInsuranceNotSupportedTip(index)"></uni-icons>
											</view>
										</view>
										<text class="insurance-hint">贵重物品建议保价，可获得更高赔付</text>
									</view>

									<!-- 保价金额输入，只在支持保价且已开启保价时显示 -->
									<view class="insurance-input-row"
										v-if="isExpressOrderSupported(index) && order.insured">
										<text class="input-label">保价金额</text>
										<view class="value-input-wrapper">
											<text class="currency-symbol">¥</text>
											<input type="digit" :value="order.insuranceValue || ''"
												@input="(e) => updateInsuranceValue(index, e)" placeholder="请输入保价金额"
												class="value-input" @tap.stop />
										</view>
									</view>

									<!-- 保价费显示，只在支持保价且已开启保价时显示 -->
									<view class="insurance-fee-row"
										v-if="isExpressOrderSupported(index) && order.insured && calculateInsuranceFee(order)">
										<text class="fee-label">保价费用</text>
										<text class="fee-value">¥{{ formatPrice(calculateInsuranceFee(order)) }}</text>
									</view>
								</view>
								<!-- 保价金额输入下方添加保价规则说明 -->
								<view class="insurance-rule-row" v-if="isExpressOrderSupported(index) && order.insured">
									<view class="rule-label">保价规则</view>
									<view class="rule-content">
										{{ getExpressBjRuleStr(index) }}
									</view>
								</view>
							</view>
						</view>
					...
					}" @click="isPaymentMethodSupported('online') && selectPaymentMethod('online', $event)">
								...
								<view class="balance-header">
									<view class="balance-info">
										<view class="service-icon">
											<view class="checkbox-wrapper"
												...
												<uni-icons type="checkmarkempty" size="16" color="#FFFFFF"
													vif="formData.useCommission"></uni-icons>
											</view>
										</view>
										<view class="service-details">
											...
											<view>
												<text class="service-desc">可用余额 ¥{{ availableBalance.toFixed(2) }}</text>
											</view>
					...
					<!-- 费用明细卡片 -->
					<view class="form-card" v-if="batchOrders.length > 0" style="animation-delay: 0.6s">
						<view class="card-header">
							<view class="title-wrapper">
						...
						<view class="form-content fee-content">
							<view class="fee-item">
								...
								<text class="fee-value">¥{{ totalBaseFee.toFixed(2) }}</text>
							</view>

							<view class="fee-item" v-if="totalInsuranceFee > 0">
								<text class="fee-label">保价费</text>
								<text class="fee-value">¥{{ totalInsuranceFee.toFixed(2) }}</text>
							</view>
							...
							<view class="fee-item total">
								...
								<text class="fee-value highlight">¥{{ getTotalFee().toFixed(2) }}</text>
							</view>
						</view>
					</view>

					<!-- 添加同意协议部分 -->
					<view class="form-card agreement-card" style="animation-delay: 0.8s" v-if="batchOrders.length > 0">
						<view class="agreement-content">
							<view class="agreement-checkbox" @click="toggleAgreement">
								<view class="checkbox-wrapper" :class="{'active': formData.agreedToTerms}">
									<uni-icons type="checkmarkempty" size="16" color="#FFFFFF"
										vif="formData.agreedToTerms"></uni-icons>
								</view>
		...
		<!-- 底部提交区域 -->
		<view class="bottom-btn-area" v-if="batchOrders.length > 0">
			<view class="batch-summary">
				<text class="batch-count">已填写 {{ batchOrders.length }} 个订单</text>
			...
			<button class="share-btn wechat" style="margin-right: 15px;margin-bottom: 1px;" @tap="showShareGuideLayer">
				<view class="btn-inner">
			
					<text>分享获佣金</text>
				</view>
			</button>
			<button class="submit-btn" :class="{ active: submitBtnActive, disabled: batchOrders.length === 0 }"
				@click="submitOrder" @touchstart="submitBtnActive = true" @touchend="submitBtnActive = false"
				@touchcancel="submitBtnActive = false">
				<text>批量提交</text>
			</button>
		</view>
	...
	export default {
		data() {
			return {
				...
				batchOrders: [], // 批量寄件订单列表


				isAllSelected: true, // 保留但始终为true
				isUseCommission: false,
				isUseMonthly: false,
				// 添加示例订单数据
				batchOrders: [],
				expressOptions: [], // 初始化为空数组而非undefined

				// 当前选择的快递公司是否支持保价
				insuranceSupported: false,

				// 不支持保价的提示信息
				insuranceNotSupportedTip: "该渠道暂时不支持保价",
				commissionFlows: [],
		...
		computed: {
			...
			finalPayAmount() {
				...
				const afterTotalFee = Number(parseFloat(this.totalFee || 0).toFixed(2));
				const commissionAmount = Number(parseFloat(this.formData.commissionAmount || 0).toFixed(2));

				// 计算最终金额
				let finalAmount = Math.max(0, afterTotalFee - commissionAmount);
			...
			totalInsuranceFee() {
				...
				return this.batchOrders
					.filter(order => order && order.selected !== false)
					.reduce((sum, order) => {
						// 只计算开启了保价且支持保价的订单的保价费
						if (order && order.insured && this.isExpressOrderSupported(order)) {
							return sum + this.calculateInsuranceFee(order);
						}
						return sum;
					}, 0);
			},
		...
		methods: {
			...
			// 查询快递选项
			queryExpressOptions(order, updateUI = true) {
				...
				const params = {
					businessParams: {
						...
						packageCount: order.packageCount || 1, // 默认为1
						...
						if (result) {
							expressApi.getExpressList().then(res => {
								if (res) {
										...
										if (Array.isArray(companyOptions) && companyOptions.length >
											...
											companyOptions.forEach(option => {
												...
												if (option.channelName !== '德邦【寄付】9折' && option
													...
													const bjRule = option.bjRule || "";
													const bjRuleStr = option.bjRuleStr ||
														`${option.channelName || companyCode}暂不支持保价，可线下与快递员协商`;
									...
									this.expressOptions = [...this.expressCompanies];

									if (this.expressCompanies.length > 0) {
										let selectedExpress = {};
										if (updateUI) {
											this.formData.expressIndex = 0;
											selectedExpress = this.expressCompanies[0];
											order.price = selectedExpress.fee;

											// 更新当前快递公司是否支持保价
											this.insuranceSupported = selectedExpress.bjRule !== "";
											this.insuranceNotSupportedTip = selectedExpress.bjRuleStr || "";

											this.updateBatchSummary();
										} else {
											selectedExpress = this.expressCompanies[this.formData
												.expressIndex];
											order.price = selectedExpress.fee;
											this.updateBatchSummary();
										...
										if (selectedExpress.originalData.payMethod == 30) {
											if (this.formData.paymentMethod !== "onlineCollect") {
			...
			selectExpress(index) {
				...
				// 检查选中的快递公司是否支持保价
				const bjRule = selectedExpress.bjRule || "";
				const bjRuleStr = selectedExpress.bjRuleStr || "该渠道暂时不支持保价";
			...
			async submitOrder() {
				...
				if (express.payMethod) {
					if (express.payMethod === 10 || express.payMethod === 20) {
						...
						let wechatId = 'PAY' + new Date().getTime() + Math.floor(Math.random() * 1000);
						let price = (parseFloat(this.getTotalFee().toFixed(2)) - parseFloat((this.formData
							.commissionAmount || 0).toFixed(2))) > 0 ? (parseFloat(this.getTotalFee().toFixed(2)) -
							parseFloat((this.formData.commissionAmount || 0).toFixed(2))).toFixed(2) : '0.00';
						const openId = uni.getStorageSync('openId');
						if (!openId) {
							const user = this.$store.getters['user/getUserInfo']
							openId = user.openId
						}
						const payResult = await wxpayApi.processPayment(openId, wechatId, price, "快递下单");
						if (payResult.success) {
							for (let i = 0; i < this.batchOrders.length; i++) {
								let orderId = 'ORDER' + new Date().getTime() + Math.floor(Math.random() * 1000);
								payStrategy[express.payMethod](this.batchOrders[i], express, startDateTime,
									endDateTime, orderId, wechatId);
							}
							orderApi.batchesAdd(this.orders).then(orderIds => {
								if (orderIds) {
					...
					} else if (express.payMethod === 30) {
						//月结支付
						if (!this.formData.useCommission && this.formData.paymentMethod === 'monthly') {
							for (let i = 0; i < this.batchOrders.length; i++) {
								let orderId = 'ORDER' + new Date().getTime() + Math.floor(Math.random() * 1000);
								this.handleMonthlyPayment(this.batchOrders[i], express, startDateTime, endDateTime,
									orderId, orderId);
							}
							orderApi.batchesAdd(this.orders).then(orderIds => {
								if (orderIds) {
									this.orderIds = orderIds;
									// this.handleOrderResult(orderIds);
									for (let j = 0; j < this.orderIds.length; j++) {
										const statement = this.createMonthlyStatement(this.orderIds[j], this
											.orders[j].orderTotalPrice);
										console.log(this.orders[j])
										this.statements.push(statement)
									}
									monthlyApi.batchesAdd(this.statements).then(statementResult => {
										this.handleOrderResult(statementResult);
									})
								}
							})
							...
							if (finalPrice === '0.00') {
								let orderId = 'ORDER' + new Date().getTime() + Math.floor(Math.random() * 1000);
								for (let i = 0; i < this.batchOrders.length; i++) {
									this.handleMonthlyPayment(this.batchOrders[i], express, startDateTime,
										endDateTime, orderId, orderId);
								}
								orderApi.batchesAdd(this.orders).then(orderIds => {
									if (orderIds) {
										this.orderIds = orderIds;
										// this.handleOrderResult(orderIds);
										for (let j = 0; j < this.orderIds.length; j++) {
											const commissionFlow = this.createCommissionFlow(this.orderIds[j],
												this.orders[j]);
											console.log(commissionFlow)
											this.commissionFlows.push(commissionFlow)
										}
										commissionFlowApi.batchesAdd(this.commissionFlows).then(
											commissionFlowResult => {
												if (commissionFlowResult) {
													userApi.getUserById(this.storeUserInfo.userId).then(
														r2 => {
															if (r2) {
								...
								if (this.formData.paymentMethod === 'monthly') {
									...
									let money = this.storeUserInfo.money;
									for (let i = 0; i < this.batchOrders.length; i++) {

										if (money > this.batchOrders[i].price) {
											commissionDeductionAmount = this.batchOrders[i].price;
											money = money - this.batchOrders[i].price
										} else {
										...
										let orderId = 'ORDER' + new Date().getTime() + Math.floor(Math.random() *
											1000);
										this.handleUseCommissionAndMonthlyPayment(this.batchOrders[i], express,
											startDateTime, endDateTime, commissionDeductionAmount, 4, orderId,
											orderId);
									}
									orderApi.batchesAdd(this.orders).then(orderIds => {
										if (orderIds) {
											...
											// this.handleOrderResult(orderIds);
											for (let j = 0; j < this.orderIds.length; j++) {
												if (this.storeUserInfo.money - this.orders[j]
													.orderTotalPrice >= 0.00) {
													const commissionFlow = this.createCommissionFlow(this
														.orderIds[j], this.orders[j].orderTotalPrice);
													this.commissionFlows.push(commissionFlow)
												} else {
													let price = this.orders[j].orderTotalPrice - this
														.storeUserInfo.money
													const commissionFlow = this.createCommissionFlow(this
														.orderIds[j], this.storeUserInfo.money);
													this.commissionFlows.push(commissionFlow)
													const statement = this.createMonthlyStatement(this
														.orderIds[j], price);
													this.statements.push(statement)
													this.storeUserInfo.money = 0.00
												}
											}
											...
											commissionFlowApi.batchesAdd(this.commissionFlows).then(
												commissionFlowResult => {
													if (commissionFlowResult) {
														monthlyApi.batchesAdd(this.statements).then(
															statementResult => {
																if (statementResult) {
																	userApi.getUserById(this
									...
									let wechatId = 'PAY' + new Date().getTime() + Math.floor(Math.random() * 1000);
									let price = (parseFloat(this.getTotalFee().toFixed(2)) - parseFloat((this.formData
										.commissionAmount || 0).toFixed(2))) > 0 ? (parseFloat(this.getTotalFee()
										.toFixed(2)) - parseFloat((this.formData.commissionAmount || 0)
									...
									if (payResult.success) {
										for (let i = 0; i < this.batchOrders.length; i++) {
											...
											let orderId = 'ORDER' + new Date().getTime() + Math.floor(Math.random() *
												1000);
											this.handleUseCommissionAndMonthlyPayment(this.batchOrders[i], express,
												startDateTime, endDateTime, commissionDeductionAmount, 1, orderId,
												wechatId);
										}
										orderApi.batchesAdd(this.orders).then(orderIds => {
											if (orderIds) {
												...
												commissionFlowApi.batchesAdd(this.commissionFlows).then(
													commissionFlowResult => {
														if (commissionFlowResult) {
															userApi.getUserById(this.storeUserInfo.userId)
																.then(r2 => {
																	if (r2) {
																		this.$store.commit(
																			'user/setUserInfo', r2)
								...
								// 		1000);
								// 		this.handleUseCommissionAndMonthlyPayment(this.batchOrders[i], express,
								// 			startDateTime, endDateTime, commissionDeductionAmount, 7, orderId,
								// 			orderId);
								// 	}
								// 	orderApi.batchesAdd(this.orders).then(orderIds => {
								// 		if (orderIds) {
							...
							if (payResult.success) {
								for (let i = 0; i < this.batchOrders.length; i++) {
									let orderId = 'ORDER' + new Date().getTime() + Math.floor(Math.random() *
										1000);
									this.handleWechatPay(this.batchOrders[i], express, startDateTime, endDateTime,
										orderId, wechatId);
								}
								orderApi.batchesAdd(this.orders).then(orderIds => {
									if (orderIds) {
							...
							for (let i = 0; i < this.batchOrders.length; i++) {
							    const promise = userApi.isRealByPhone(this.batchOrders[i].receiverMobile).then(res => {
							        if(res) {
							            if (res === '0') {
							                let orderId = 'ORDER' + new Date().getTime() + Math.floor(Math.random() * 1000);
							                this.handleOnlineCollect(this.batchOrders[i], express, startDateTime, endDateTime, orderId, orderId);
							            } else {
							                errorNumber = errorNumber + this.batchOrders[i].receiverMobile + ';';
							...
							Promise.all(promises).then(() => {
							    if(this.orders.length > 0) {
							        ...
							        let errortTile = '部分下单失败，请及时通知收件人'+errorNumber+'注册并实名"神州邮"';
							        uni.showToast({
							            title: errortTile,
							            icon: 'none',
			...
			//处理微信支付
			async handleOnlineCollect(order, express, startDateTime, endDateTime, orderId, wechatId) {
				uni.showLoading({
					title: '下单中...',
					mask: true
				});
			...
			// 公共方法 - 创建基础请求数据
			createBaseRequestData(order, express, startDateTime, endDateTime, thirdNo) {
				console.log(order)
				const baseData = {
			...
			// 公共方法 - 创建订单数据
			createOrderData(order, express, paymentMethod, orderStatus, startDateTime, endDateTime,
				commissionDeductionAmount, orderPrice, orderId, wechatId) {
				let insuranceFee = this.formatPrice(this.calculateInsuranceFee(order))
				return {
					orderId,
					wechatId,
			...
			// 辅助方法 - 判断是否有体积信息
			hasVolumeDimensions(order) {
				return order.length && order.width && order.height;
			},

			// 处理线下支付（寄付/到付）
			async handleOfflinePayment(order, express, startDateTime, endDateTime, orderId, wechatId) {
				...
				try {
					...
					const expressRes = await expressApi.createExpress(requestData);
					if (!expressRes) throw new Error('快递下单失败');
					this.expressOrders.push(requestData)
					const orderData = this.createOrderData(
						order,
						express,
			...
			async handleUseCommissionAndMonthlyPayment(order, express, startDateTime, endDateTime,
				commissionDeductionAmount, payFuntion, orderId, wechatId) {
				uni.showLoading({
					title: '下单中...',
					mask: true
				});
			...
			async handleUseCommission(order, express, startDateTime, endDateTime) {
				uni.showLoading({
					title: '下单中...',
					mask: true
				});
				try {
					...
					const commissionFlow = await this.createCommissionFlow(orderRes);
					await this.$store.dispatch('user/updateUserInfo')
					this.handleOrderResult(commissionFlow);
				} catch (error) {
					console.error('月结支付失败:', error);
					uni.showToast({
						title: '下单失败',
			...
			// 处理月结支付
			async handleMonthlyPayment(order, express, startDateTime, endDateTime, orderId, wechatId) {
				uni.showLoading({
					title: '下单中...',
					mask: true
				});
			...
			//处理微信支付
			async handleWechatPay(order, express, startDateTime, endDateTime, orderId, wechatId) {
				uni.showLoading({
					title: '下单中...',
					mask: true
				});
			...
			// 创建月结账单
			createMonthlyStatement(orderId, orderTotalPrice) {
				const statement = {
					userId: this.storeUserInfo.userId,
			...
			//创建微信支付流水记录
			createPaymentFlow(orderId, orderTotalPrice, merchantOrderId, transactionId) {
				const paymentFlow = {
					userId: this.storeUserInfo.userId,
			...
			// 创建佣金抵扣账单
			createCommissionFlow(orderId, orderTotalPrice) {
				this.storeUserInfo.money = this.storeUserInfo.money - orderTotalPrice;
				let balance = this.storeUserInfo.money;
			...
			// 计算保险费
			calculateInsuranceFee(order) {
				if (!order || !order.insured || !order.insuranceValue) {
					return 0;
				}

				const expressIndex = order.expressIndex || 0;
				let insuranceFee = 0; // 默认保价费

				// 获取保价金额
				const insValue = parseFloat(order.insuranceValue);
				if (isNaN(insValue) || insValue <= 0) return 0;

				// 从快递公司配置中获取保价规则
				if (this.expressCompanies &&
					this.expressCompanies.length > expressIndex &&
					this.expressCompanies[expressIndex].bjRule) {

					try {
						// 尝试解析bjRule JSON字符串
						const bjRuleStr = this.expressCompanies[expressIndex].bjRule;
						let bjRules = JSON.parse(bjRuleStr);

						// 确保bjRules是数组
						if (!Array.isArray(bjRules)) {
							// 如果不是数组，可能是单一规则或者百分比
							const rateNum = parseFloat(bjRuleStr);
							if (!isNaN(rateNum)) {
								// 按照百分比计算
						...
						for (const rule of bjRules) {
							const start = parseFloat(rule.start || 0);
							const end = parseFloat(rule.end || 0);
			...
			// 验证保价金额
			validateInsuranceValue(e) {
				let value = e.detail ? e.detail.value : this.formData.insuranceValue;
			...
			onLoad() {
				...
				// 检查是否有缓存数据
				const cachedOrders = uni.getStorageSync('batchOrders');
				if (cachedOrders) {
					try {
						const parsedOrders = cachedOrders;
						// 只有当缓存中有数据时才替换示例数据
			...
			isPaymentMethodSupported(method) {
				...
				const supported = this.supportedPayMethods.includes(method);
			...
			editOrder(index) {
				// 获取要编辑的订单
				const order = this.batchOrders[index];

				// 方法一：通过URL参数传递（适合数据量较小的情况）
				// 将订单数据转换为JSON字符串并编码后通过URL传递
				uni.navigateTo({
					url: '/views/express/batch-order-edit?orderData=' + encodeURIComponent(JSON.stringify(order)) +
						'&index=' + index
				});
			...
			deleteOrder(index) {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.batchOrders.splice(index, 1);
							this.updateBatchSummary();
							this.saveBatchOrdersToStorage();
							for (let i = 0; i < this.batchOrders.length; i++) {
			...
			// 更新批量订单方法 - 供编辑页面调用
			updateBatchOrder(index, orderData) {
				if (index >= 0 && index < this.batchOrders.length) {
					// 更新订单数据但保持选中状态
					this.batchOrders[index] = {
						...orderData,
						selected: true
					};
			...
			// 从本地存储加载批量订单
			loadBatchOrdersFromStorage() {
				try {
					const storageData = uni.getStorageSync('batchOrders');
					if (storageData) {
			...
			// 添加到methods中
			formatInsuranceFee(insuranceValue) {
				if (!insuranceValue) return '0.00';
				// 保价费率为1%
				const fee = parseFloat(insuranceValue) * 0.01;
				return fee.toFixed(2);
			},

			// 在methods中添加
			// 切换订单保价状态
			toggleOrderInsurance(index) {
				if (index >= 0 && index < this.batchOrders.length) {
					// 设置默认值
			...
			isExpressOrderSupported(index) {
				...
				const order = this.batchOrders[index];
				const expressIndex = this.formData.expressIndex || 0;
				// console.log(this.expressOptions)
				if (this.expressOptions.length == 0 ||
					this.expressOptions.length <= expressIndex) {
					return false;
				}

				const express = this.expressOptions[expressIndex];
				// console.log(express)
			...
			// 安全地显示不支持保价的提示
			showInsuranceNotSupportedTip(index) {
				// 防御性检查
				if (!this.batchOrders || index < 0 || index >= this.batchOrders.length) {
					uni.showModal({
						title: '保价规则说明',
				...
				let ruleText = '该快递不支持保价服务';

				if (this.expressCompanies &&
					this.expressCompanies[expressIndex] &&
					this.expressCompanies[expressIndex].bjRuleStr) {
			...
			getExpressBjRuleStr(index) {
				...
				if (!this.expressCompanies ||
					expressIndex >= this.expressCompanies.length ||
					!this.expressCompanies[expressIndex].bjRule) {
					return '保价费率：1%（默认）';
				}
				...
				try {
					...
					if (Array.isArray(bjRules) && bjRules.length > 0) {
						...
						for (const rule of bjRules) {
							...
							const end = rule.end || '以上';

							if (rule.bjPrice !== undefined) {
								// 固定价格规则
								ruleText = `保价金额${start}-${end === 0 ? '以上' : end}元，固定收费${rule.bjPrice}元`;
							} else if (rule.bjRate !== undefined) {
								// 费率规则
								const ratePercent = (rule.bjRate * 100).toFixed(2);
								ruleText = `保价金额${start}-${end === 0 ? '以上' : end}元，费率${ratePercent}%`;
							}
			...
			// 根据索引更新订单
			updateOrderAtIndex(index, updatedOrder) {
				if (index >= 0 && index < this.batchOrders.length) {
					// 使用Vue.set确保响应式更新
...
<style lang="scss">
	...
	.send-container {
		...
		.form-card {
			...
			.card-header {
				...
				.card-title {
					background: linear-gradient(to right, #FF6B01, #FF8F00);
		...
		.batch-order-item {
			...
			&::before {
				...
				width: 80rpx;
				height: 80rpx;
				background: linear-gradient(135deg, transparent 50%, rgba(255, 107, 1, 0.05) 50%);
		...
		.add-btn {
			...
			padding: 10rpx 24rpx;
			border-radius: 40rpx;
			box-shadow: 0 6rpx 15rpx rgba(255, 107, 1, 0.2);
			transition: all 0.3s ease;
		...
		.checkbox-wrapper {
			...
			&.active {
				...
				box-shadow: 0 4rpx 10rpx rgba(255, 107, 1, 0.2);
			}
		}
		...
		.empty-state {
			...
			.empty-icon {
				...
				height: 240rpx;
				opacity: 0.9;
				filter: drop-shadow(0 8rpx 15rpx rgba(0, 0, 0, 0.1));
			...
			.empty-btn {
				...
				&:active {
					transform: scale(0.95);
					box-shadow: 0 4rpx 10rpx rgba(255, 107, 1, 0.15);
				}
		...
		@keyframes pulse {
			...
			50% {
				box-shadow: 0 8rpx 30rpx rgba(255, 107, 1, 0.5);
			}
	...
	.order-addresses.vertical {
		...
		.order-address {
			...
			&.receiver-section {
				.address-tag {
					background: linear-gradient(135deg, #3A86FF, #5C9DFF);
					box-shadow: 0 4rpx 8rpx rgba(58, 134, 255, 0.2);
				}
			}
		}
		...
		.address-divider {
			...
			.divider-line {
				flex: 1;
				height: 1rpx;
				background: linear-gradient(to right, rgba(204, 204, 204, 0.5), rgba(204, 204, 204, 0));

				&:last-child {
					background: linear-gradient(to left, rgba(204, 204, 204, 0.5), rgba(204, 204, 204, 0));
				}
			}
	...
	.insurance-section {
		...
		box-shadow: 0 2rpx 8rpx rgba(255, 107, 1, 0.05);
		...
		.insurance-input-row {
			...
			.value-input-wrapper {
				...
				&:focus-within {
					border-color: #FF6B01;
					box-shadow: 0 0 0 2px rgba(255, 107, 1, 0.1);
				}
