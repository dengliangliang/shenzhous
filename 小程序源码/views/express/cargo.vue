					...
					<view class="form-card" style="animation-delay: 0.2s">
						...
						<view class="form-content">
							...
							<view class="input-row">
								...
								<input type="digit" v-model="formData.estimatedItemWeight"
									placeholder="请输入物品重量(不低于20kg)" class="input weight-input"
									@input="validateWeightInput" @blur="checkAndQueryExpress" maxlength="5" />
		...
		<view class="bottom-btn-area">
			<view class="total-fee">
				...
				<view class="fee-amount-wrapper">
					...
					<!-- 直接计算显示，不依赖计算属性 -->
					<text class="fee-amount">¥{{ (parseFloat((totalFee || 0).toFixed(2))
						 - parseFloat((formData.commissionAmount || 0).toFixed(2))) > 0 ? (parseFloat((totalFee || 0).toFixed(2))
						  - parseFloat((formData.commissionAmount || 0).toFixed(2))).toFixed(2) : '0.00' }}</text>
				</view>
			</view>
			...
			<button class="submit-btn" :class="{ active: submitBtnActive }" @click="submitOrder"
				@touchstart="submitBtnActive = true" @touchend="handleTouchEnd" @touchcancel="submitBtnActive = false"
				:disabled="submitting">
	...
	export default {
		...
		methods: {
			...
			checkAndQueryExpress() {
				// 检查重量是否符合大件标准
				const weight = parseFloat(this.formData.estimatedItemWeight || 0);
